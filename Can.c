...
enum _flexcan_mb_code_rx
{
    ...
    kCAN_RxMbRanswer  = 0xA, /*!< A frame was configured to recognize a Remote Request Frame and transmit a
 ...
 * clock and bit rate, if not match, the final output bit rate may not equal the bitRate_Bps value.
 *
 * base: CAN peripheral base address.
 * sourceClock_Hz: Source Clock in Hz.
 * bitRate_Bps: Bit rate in Bps.
 * timingConfig: FlexCAN timingConfig.
 */
static void CAN_SetBitRate(CAN_Type *base,
 ...
 * retval kStatus_Fail    - Tx Message Buffer is currently in use.
 */
status_t CAN_TransferSendBlocking(CAN_Type *base, uint8_t mbIdx, Can_PduType *pTxFrame) {
    status_t status;
    /* Write Tx Message Buffer to initiate a data sending. */
    if (kStatus_Success == CAN_Write(base, mbIdx, (const Can_PduType *)(uint32_t)pTxFrame)) {
/* Wait until CAN Message send out. */
#if (defined(FSL_FEATURE_FLEXCAN_HAS_EXTENDED_FLAG_REGISTER)) && (FSL_FEATURE_FLEXCAN_HAS_EXTENDED_FLAG_REGISTER > 0)
        uint64_t u64flag = 1;
        while (0U == CAN_GetMbStatusFlags(base, u64flag << mbIdx))
#else
        uint32_t u32flag = 1;
        while (0U == CAN_GetMbStatusFlags(base, u32flag << mbIdx))
#endif
        { }
/* Clean Tx Message Buffer Flag. */
#if (defined(FSL_FEATURE_FLEXCAN_HAS_EXTENDED_FLAG_REGISTER)) && (FSL_FEATURE_FLEXCAN_HAS_EXTENDED_FLAG_REGISTER > 0)
        CAN_ClearMbStatusFlags(base, u64flag << mbIdx);
#else
        CAN_ClearMbStatusFlags(base, u32flag << mbIdx);
#endif
 ...
 */
bool CAN_CalculateImprovedTimingValues(CAN_Type *base,
                                       uint32_t bitRate,
                                       uint32_t sourceClock_Hz,
 ...
 */
static void CAN_GetSegments(CAN_Type *base,
                            uint32_t bitRate,
                            uint32_t tqNum,
                            can_timing_config_t *pTimingConfig) {
    uint32_t ideal_sp;
...
void Can_Init(const Can_ConfigType *pConfig) {
    ...
    if (CAN_CalculateImprovedTimingValues(EXAMPLE_CAN, pConfig->baudRate, EXAMPLE_CAN_CLK_FREQ,
											  &timing_config)) {
    ...
    CAN_Enable(pConfig->CAN_base, true);
    /* Reset to known status. */
    CAN_Reset(pConfig->CAN_base);
#if (defined(FSL_FEATURE_FLEXCAN_HAS_MEMORY_ERROR_CONTROL) && FSL_FEATURE_FLEXCAN_HAS_MEMORY_ERROR_CONTROL)
